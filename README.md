...
主机列表：`↑` `↓` / `j` 上 `k` 下 选择，`Enter` 连接（或进入本机），`r` 扫描，`d` 忘掉，`q` 退出。

tmux 列表：`↑` `↓` / `j` 上 `k` 下 选择，`Enter` 进入，`n` 新建，`e` 重命名，`d` 删除，`p` 常驻，`X` 清闲（空闲且非常驻），`h` 换机器，`o` 占用优先/按时间，`/` 包含，`!` 排除，`f` 开关上次筛选，`s` 普通 shell（`exit` 返回），`q` 退出。
