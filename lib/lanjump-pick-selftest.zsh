...
pick_selftest() {
  ...
  # #87: same window, current pane idle, first pane busy; resume the idle pane only.
  : >"$tmux_log"
  ...
  if [[ $restore_log == *'respawn-pane -t %1 '* ]]; then
    print -u2 "FAIL resume/split-idle killed first (busy) pane got=$(printf %q "$restore_log")"
    (( fails++ ))
  fi
