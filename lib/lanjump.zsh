...
upsert_host() {
  ...
  local old_user="" old_hn="" old_port=""
  local -i ssh_existed=0 old_removed=0
  if [[ -z ${_LANJUMP_HOSTS_LOCKED:-} ]]; then
    _LANJUMP_HOSTS_LOCKED=1
  ...
  if id=$(alloc_ssh_id "$alias" "$mac" "$ip" "$idx"); then
    ...
    if [[ -n $old_id && $old_id != "$id" ]] && ! ssh_id_taken "$old_id" "$idx"; then
      ...
      if [[ -f $SSH_CONFIG ]] && grep -qF "$begin" "$SSH_CONFIG" 2>/dev/null; then
        old_hn=$(awk -v b="$begin" -v e="$end" '
          $0 == b { p = 1; next }
          $0 == e { p = 0 }
          p && $1 == "HostName" { print $2; exit }
        ' "$SSH_CONFIG")
        old_user=$(awk -v b="$begin" -v e="$end" '
          $0 == b { p = 1; next }
          $0 == e { p = 0 }
          p && $1 == "User" { print $2; exit }
        ' "$SSH_CONFIG")
        old_port=$(awk -v b="$begin" -v e="$end" '
          $0 == b { p = 1; next }
          $0 == e { p = 0 }
          p && $1 == "Port" { print $2; exit }
        ' "$SSH_CONFIG")
